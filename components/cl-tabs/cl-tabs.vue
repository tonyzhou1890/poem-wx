  ...
  export default {
    ...
    methods: {
      ...
      movExtendAni(ratio, yRatio) {
        ...
        let translateX = Math.abs(this.sliderMove * ratio) > Math.abs(maxTranslateX) ? maxTranslateX : this.sliderMove *
          ratio
      ...
      textAni(ratio, yRatio) {
        ...
        let scale = this.scale + (1 - this.scale) * (Math.abs(yRatio)) < 1 ? 1 : this.scale + (1 - this.scale) * (Math.abs(
          yRatio))
